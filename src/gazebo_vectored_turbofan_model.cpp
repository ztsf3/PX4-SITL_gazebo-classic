...
void GazeboVectoredTurbofanModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  //     thrustChannel  -> Fz
  //     vectorYChannel -> Fy
  // - maxThrust / maxVectorAngleRad 保留读取以兼容旧 SDF，
  //   但当前版本不再参与力计算。
  getSdfParam<std::string>(_sdf, "commandSubTopic", command_sub_topic_, command_sub_topic_);
  getSdfParam<std::string>(_sdf, "forceVisualTopic", force_visual_topic_, force_visual_topic_);
  getSdfParam<double>(_sdf, "forceVisualPubInterval", force_visual_pub_interval_, force_visual_pub_interval_);
...
void GazeboVectoredTurbofanModel::CommandCallback(const CommandMotorSpeedPtr &msg) {
  // 函数作用：
  // - 从 CommandMotorSpeed 中读取本喷口绑定的 3 路输入；
  // - 直接解释为当前 link 局部坐标系下的三轴分力。
  // 输入：msg（完整执行器数组）。
  // 输出：无（更新 force_x_cmd_ / force_y_cmd_ / force_z_cmd_）。
  ...
  // - 不做限幅；
  // - 不做归一化到物理量的转换；
  // - 上游发送什么值，这里就按什么值作为力分量使用。
  force_x_cmd_ = channel(vector_x_channel_);
  force_y_cmd_ = channel(vector_y_channel_);
  force_z_cmd_ = channel(thrust_channel_);
}

void GazeboVectoredTurbofanModel::OnUpdate(const common::UpdateInfo &_info) {
  // 函数作用：
  // - 根据 dt 更新一阶惯性状态；
  // - 将三轴分力命令平滑为当前施加的三维力矢量；
  // - 向 Gazebo 物理引擎施加力；
  // - 按需发布力可视化消息。
  // 输入：_info（包含当前仿真时间）。
  // 输出：无（通过 AddLinkForce 和 Publish 产生效果）。
  ...
  // 一阶惯性离散化：x(k+1)=x(k)+alpha*(u-x(k))。
  // 当 time_constant_ <= 0 时，视为不滤波，直接跟随命令。
  double alpha = 1.0;
  if (time_constant_ > 0.0) {
    alpha = ignition::math::clamp(dt / time_constant_, 0.0, 1.0);
  }

  force_x_state_ += alpha * (force_x_cmd_ - force_x_state_);
  force_y_state_ += alpha * (force_y_cmd_ - force_y_state_);
  force_z_state_ += alpha * (force_z_cmd_ - force_z_state_);

  // 三轴分力状态 -> 三维力矢量。
  // 这里的 body_force 按当前 link 的局部坐标系解释。
  const ignition::math::Vector3d body_force(
      force_x_state_,
      force_y_state_,
      force_z_state_);
